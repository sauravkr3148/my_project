        ...
        unsafe {
            ...
            let result = {
                if let Some(gdi_capturer) = &self.gdi_capturer {
                    ...
                    if rotate == kRotate0 {
                        let dest_stride = self.width * 4;
                        let pitch = r.1 as usize;
                        self.rotated.resize(dest_stride * self.height, 0);
                        let src_ptr = r.0 as *const u8;
                        let dst_ptr = self.rotated.as_mut_ptr();
                        for row in 0..self.height {
                            let src_offset = row * pitch;
                            let dst_offset = row * dest_stride;
                        ...
                        if gdi::is_cursor_enabled() {
                            let origin = self.display.origin();
                            let width = self.width;
                            let height = self.height;
                            let buffer = self.rotated.as_mut_slice();
                            draw_cursor_on_buffer(buffer, width, height, origin);
                        }
                        &self.rotated[..]
                    } else {
...
fn draw_cursor_on_buffer(buffer: &mut [u8], width: usize, height: usize, origin: (i32, i32)) {
    ...
    use winapi::um::winuser::{GetCursorInfo, GetDC, GetIconInfo, ReleaseDC, CURSORINFO, ICONINFO};

    unsafe {
        const DI_NORMAL: u32 = 0x0003;
        let mut cursor_info: CURSORINFO = mem::zeroed();
        cursor_info.cbSize = mem::size_of::<CURSORINFO>() as _;
        if GetCursorInfo(&mut cursor_info) == 0 {
            return;
        }
        let flags_showing = 0x00000001; // CURSOR_SHOWING
        if cursor_info.flags & flags_showing == 0 {
            return;
        }
        ...
        let mut cursor_height = 0;
        if !icon_info.hbmColor.is_null() {
            if winapi::um::wingdi::GetObjectW(
                icon_info.hbmColor as _,
                mem::size_of::<BITMAP>() as _,
                &mut bmp as *mut _ as *mut WinCvoid,
            ) != 0
            {
                cursor_width = bmp.bmWidth;
                cursor_height = bmp.bmHeight;
            }
        }
        ...
        let cursor_x = cursor_info.ptScreenPos.x - origin_x;
        let cursor_y = cursor_info.ptScreenPos.y - origin_y;

        let left = cursor_x - icon_info.xHotspot as i32;
        let top = cursor_y - icon_info.yHotspot as i32;
        let right = left + cursor_width;
        let bottom = top + cursor_height;

        let frame_width = width as i32;
        let frame_height = height as i32;

        let region_left = left.max(0).min(frame_width);
        let region_top = top.max(0).min(frame_height);
        let region_right = right.max(0).min(frame_width);
        let region_bottom = bottom.max(0).min(frame_height);
        ...
        let offset_x = region_left - left;
        let offset_y = region_top - top;
        let region_width = (region_right - region_left) as i32;
        let region_height = (region_bottom - region_top) as i32;
        ...
        let mut bmi = BITMAPINFO {
            bmiHeader: BITMAPINFOHEADER {
                ...
                biWidth: region_width,
                biHeight: -region_height, // top-down
                biPlanes: 1,
                biBitCount: 32,
                biCompression: BI_RGB,
                biSizeImage: (region_width * region_height * 4) as _,
                biXPelsPerMeter: 0,
                biYPelsPerMeter: 0,
                biClrUsed: 0,
                biClrImportant: 0,
        ...
        let mut bits: *mut WinCvoid = ptr::null_mut();
        ...
        let frame_stride = width * bytes_per_pixel;
        let region_stride = region_width as usize * bytes_per_pixel;
        let bits_ptr = bits as *mut u8;

        for row in 0..region_height as usize {
            let src_y = region_top as usize + row;
            let src_offset = src_y * frame_stride + region_left as usize * bytes_per_pixel;
            let dst_offset = row * region_stride;
            ptr::copy_nonoverlapping(
                buffer.as_ptr().add(src_offset),
                bits_ptr.add(dst_offset),
                region_stride,
            );
        }

        let old_obj = SelectObject(mem_dc, hbitmap as _);
        ...
        for row in 0..region_height as usize {
            let dst_y = region_top as usize + row;
            let dst_offset = dst_y * frame_stride + region_left as usize * bytes_per_pixel;
            let src_offset = row * region_stride;
            ptr::copy_nonoverlapping(
                bits_ptr.add(src_offset),
