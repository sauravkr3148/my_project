use std::mem::size_of;
use winapi::{
    shared::windef::{HBITMAP, HDC, POINT},
    um::wingdi::{
        BitBlt,
    ...
    um::winuser::{DrawIconEx, GetCursorInfo, CURSORINFO, CURSOR_SHOWING},
};

// Import CLIENT_STATE for cursor visibility check
use once_cell::sync::OnceCell;
use std::sync::Arc;

// Forward declaration - will be set by the main application
static CLIENT_STATE: OnceCell<Arc<dyn ClientStateProvider>> = OnceCell::new();

// Trait to provide cursor visibility state
pub trait ClientStateProvider: Send + Sync {
    fn is_cursor_visible(&self) -> bool;
}

// Function to set the client state provider
pub fn set_client_state_provider(provider: Arc<dyn ClientStateProvider>) {
    let _ = CLIENT_STATE.set(provider);
}

pub fn is_cursor_enabled() -> bool {
    CLIENT_STATE
        .get()
        .map(|provider| provider.is_cursor_visible())
        .unwrap_or(true)
}
...
impl CapturerGDI {
    ...
    pub fn frame(&self, data: &mut Vec<u8>) -> Result<(), Box<dyn std::error::Error>> {
        unsafe {
            ...
            // Only draw cursor if it's enabled in the client state
            if let Some(client_state) = CLIENT_STATE.get() {
                if client_state.is_cursor_visible() {
                    if let Err(e) = self.draw_cursor_on_frame() {
                        // Don't fail the entire capture if cursor drawing fails
                        eprintln!("Warning: Failed to draw cursor: {}", e);
                    }
                }
            }
    ...
    /// Draw cursor on the captured frame to ensure visibility in remote clients
    fn draw_cursor_on_frame(&self) -> Result<(), Box<dyn std::error::Error>> {
        unsafe {
            ...
            if GetCursorInfo(&mut cursor_info) != 0 && (cursor_info.flags & CURSOR_SHOWING) != 0 {
                // Draw the cursor icon on our device context
                let result = DrawIconEx(
                    ...
                    0,                    // No animation step
                    std::ptr::null_mut(), // No flicker-free drawing
                    0x0003,               // DI_NORMAL value (0x0003)
                );

                if result == 0 {
                    // Cursor drawing failed, but don't fail the entire capture
                    // This is a non-critical error
                    eprintln!("Warning: Failed to draw cursor on captured frame");
                }
            }
